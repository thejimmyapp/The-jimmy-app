...
export default function App() {
  ...
  useEffect(() => {
    const handleReplayNavigation = (event: KeyboardEvent) => {
      ...
      const current = useCoachStore.getState();
      if (!current.game || current.mode !== "review" || isTyping) return;
      if (event.key === "Tab" && current.guestMatch) {
        event.preventDefault();
        setActiveReviewBoard((board) => board === "A" ? "B" : "A");

...
import { BarChart3, BookOpen, Bot, Check, Copy, ExternalLink, FileInput, Flag, Home, LockKeyhole, LogOut, Palette, Radio, Redo2, RotateCcw, Settings, ShieldCheck, StickyNote, Swords, Undo2, UserRoundPlus, Users, X } from "lucide-react";
...
import { acceptAnalysisAcknowledgement, clearGuestProgress, emptyGuestProgress, hasAnalysisAcknowledgement, isCapabilityLocked, lessonStorageId, loadGuestProgress, qualifyingGameCount, savedLessonFrom, savedMomentKey, storeGuestProgress, type CapabilityKey, type GuestProgress, type SavedLesson, type SavedMoment } from "./guestProgress";
import { EMPTY_GUEST_SESSION, type GuestSessionIdentity } from "./guestChrome";
import { captureMomentContext, matchForSavedMoment, momentPermalinkPath, playerNamesForMoment, resolveMomentAddress, savedMomentFromCapture, type MomentCapture } from "./learningMoments";
import { completeGuestQuestIfReady, QUEST_TARGET_MOMENTS, questRemainingSeconds, startGuestQuest } from "./quest";
...
import { studyUiRequested } from "./parity/workspace/studyFlag";
...
interface InitialMomentPermalink {
  gameId: string | null;
  moveAddress: string;
}

const initialMomentPermalink = (): InitialMomentPermalink | null => {
  const params = new URLSearchParams(location.search);
  if (!params.has("moment")) return null;
  return { gameId: params.get("game"), moveAddress: params.get("moment") ?? "" };
};
...
export default function App() {
  ...
  const [view, setView] = useState<"review" | "stats" | "notes">("review");
  ...
  const [momentPermalinkError, setMomentPermalinkError] = useState<string | null>(null);
  ...
  const [studyUiEnabled, setStudyUiEnabled] = useState(studyUiRequested);
  const expiryRunningRef = useRef(false);
  const storedGuestGamesRef = useRef(new Map<number, number>());
  const analysisResolverRef = useRef<((accepted: boolean) => void) | null>(null);
  const [requestedMomentPermalink] = useState(initialMomentPermalink);
  ...
  const guestSessionQuery = useQuery({ queryKey: ["guest-session"], queryFn: api.guestSession });
  const accountQuery = useQuery({ queryKey: ["account"], queryFn: api.accountMe, enabled: guestSessionQuery.data?.completed === true });
  ...
  useEffect(() => {
    if (!guestSessionQuery.data) return;
    updateGuestProgress((current) => completeGuestQuestIfReady(current, guestSessionQuery.data.saved_moment_count));
  }, [guestSessionQuery.data, updateGuestProgress]);
  ...
  const resetGuestSession = useCallback(() => {
    ...
    storedGuestGamesRef.current.clear();
    clearGuestProgress();
  ...
  useEffect(() => {
    const handleReplayNavigation = (event: KeyboardEvent) => {
      const target = event.target;
      const isTyping = target instanceof HTMLElement && (target.isContentEditable || target.matches("input, textarea, select"));
      const current = useCoachStore.getState();
      if (studyUiRequested() || !current.game || current.mode !== "review" || isTyping || (event.key !== "ArrowLeft" && event.key !== "ArrowRight")) return;
      event.preventDefault();
      const max = Math.max(0, current.game.timeline.length ? current.game.timeline.length - 1 : current.game.positions_a.length - 1);
      const next = Math.max(0, Math.min(max, current.globalPly + (event.key === "ArrowLeft" ? -1 : 1)));
      current.seek(next);
      sendRoomEvent("timeline.seek", { global_ply: next });
    };
    window.addEventListener("keydown", handleReplayNavigation);
    return () => window.removeEventListener("keydown", handleReplayNavigation);
  }, []);
  ...
  const selectGuestMatch = useCallback(async (match: NormalizedMatch) => {
    ...
    activateGuestReplay(source.match, replay.game);
  }, [activateGuestReplay]);

  useEffect(() => {
    if (!requestedMomentPermalink) return;
    let cancelled = false;
    const fail = (message: string) => {
      if (!cancelled) setMomentPermalinkError(message);
    };
    const resolvePermalink = async () => {
      const moveAddress = requestedMomentPermalink.moveAddress;
      if (!isMoveAddress(moveAddress)) {
        fail(`Moment address "${moveAddress || "(empty)"}" is malformed. Expected a move number followed by A, a, B, or b.`);
        return;
      }

      const rawGameId = requestedMomentPermalink.gameId ?? "";
      if (!/^[1-9]\d*$/.test(rawGameId)) {
        fail(`Moment address "${moveAddress}" has an unknown or invalid bridge game id "${rawGameId || "(missing)"}".`);
        return;
      }
      const bridgeGameId = Number(rawGameId);
      if (!Number.isSafeInteger(bridgeGameId)) {
        fail(`Moment address "${moveAddress}" has an unknown bridge game id "${rawGameId}".`);
        return;
      }

      let matchupList: Awaited<ReturnType<typeof api.guestMatchups>>;
      try {
        matchupList = await queryClient.fetchQuery(guestMatchupsQuery);
      } catch {
      ...
      let replay: ReturnType<typeof reconstructGuestMatch>;
      try {
        const source = await api.chessComMatchReplay(match.game_ids.A);
        if (source.match.game_ids.A !== match.game_ids.A || source.match.game_ids.B !== match.game_ids.B) {
          throw new MatchReconstructionError("invalid_partner_link", "The selected matchup changed while its replay was loading.");
        }
        replay = reconstructGuestMatch(source);
      } catch {
        fail(`Moment address "${moveAddress}" could not load bridge game ${bridgeGameId} as a coupled guest replay.`);
        return;
      }
      if (cancelled) return;

      const resolution = resolveMomentAddress(replay.game, moveAddress);
      if (!resolution.ok) {
        const frameProblem = resolution.reason === "multiple_frames"
          ? "identifies multiple coupled replay frames"
          : resolution.reason === "zero_frames"
            ? "does not identify any coupled replay frame"
            : "is malformed";
        fail(`Moment address "${moveAddress}" ${frameProblem} in bridge game ${bridgeGameId}. No position was opened.`);
        return;
      }

      setMomentPermalinkError(null);
      activateGuestReplay(match, replay.game);
      useCoachStore.getState().seek(resolution.globalPly);
      sendRoomEvent("timeline.seek", { global_ply: resolution.globalPly });
    };
    void resolvePermalink();
    return () => { cancelled = true; };
  }, [activateGuestReplay, queryClient, requestedMomentPermalink]);

  const refreshGuestSession = async () => {
    await queryClient.invalidateQueries({ queryKey: ["guest-session"], refetchType: "none" });
    const refreshed = await api.guestSession();
    queryClient.setQueryData<GuestSessionIdentity>(["guest-session"], refreshed);
    return refreshed;
  };

  const momentErrorMessage = (error: unknown) => {
    if (error instanceof ApiError && error.code === "daily_moment_cap_reached") {
      return "daily_moment_cap_reached: Daily moment cap reached. This moment was not saved.";
    }
    if (error instanceof ApiError && error.code) return `${error.code}: ${error.message}`;
    return error instanceof Error ? error.message : "The moment could not be saved.";
  };

  const saveLearningMoment = async ({ moveToken, glyph, alternativeMove, writtenAnswer }: { moveToken: string; glyph: SavedMoment["glyph"]; alternativeMove: string; writtenAnswer: string }) => {
    const capture = momentCapture;
    if (!capture || !store.guestMatch || moveToken !== capture.moveToken || momentSavePending) return;
    setMomentSavePending(true);
    setMomentPersistenceError(null);
    try {
      const externalGameId = capture.matchIds.A;
      let storedGameId = storedGuestGamesRef.current.get(externalGameId);
      if (!storedGameId) {
        const stored = await api.storeChessComGuestMatch(externalGameId);
        if (!Number.isSafeInteger(stored.game_id) || stored.game_id <= 0) throw new Error("Stored game response did not contain a valid game_id.");
        storedGameId = stored.game_id;
        storedGuestGamesRef.current.set(externalGameId, storedGameId);
      }
      ...
      const moment = savedMomentFromCapture(capture, glyph, alternativeMove, writtenAnswer, privateMomentId);
      ...
      try {
        ...
        setMomentPersistenceError(`Moment saved, but its live count could not be refreshed: ${momentErrorMessage(error)}`);
      }
    } catch (error) {
      setMomentPersistenceError(momentErrorMessage(error));
    } finally {
      setMomentSavePending(false);
    }
  };

  const removeSavedMoment = async (key: string) => {
    const moment = guestProgress.savedMoments.find((item) => savedMomentKey(item) === key);
    if (!moment?.serverId) {
      setMomentPersistenceError("moment_not_found: This cached moment has no server record and was not deleted.");
      return;
    }
    setMomentPersistenceError(null);
    try {
      await api.deleteMoment(moment.serverId);
      updateGuestProgress((current) => ({
        ...current,
        savedMoments: current.savedMoments.filter((item) => savedMomentKey(item) !== key),
      }));
      await refreshGuestSession();
    } catch (error) {
      setMomentPersistenceError(momentErrorMessage(error));
  ...
  const copySavedMomentLink = async (moment: SavedMoment) => {
    const path = momentPermalinkPath(moment.matchIds.A, moment.moveToken ?? "");
  ...
  const wizardSourceFrame = momentCapture && store.game ? store.game.timeline[momentCapture.ply - 1] : null;
  const wizardPosition = wizardSourceFrame && momentCapture
    ? (momentCapture.boardId === "A" ? wizardSourceFrame.board_a : wizardSourceFrame.board_b)
    : null;
  const wizardPairedPosition = wizardSourceFrame && momentCapture
    ? (momentCapture.boardId === "A" ? wizardSourceFrame.board_b : wizardSourceFrame.board_a)
    : null;
  const wizardOrientation = momentCapture?.boardId === stagedSourceBoard ? stagedOrientation : dockOrientation;
  const wizardPlayerBottom = momentCapture ? boardPlayers[momentCapture.boardId][wizardOrientation] : "Player";
  const wizardPlayerTop = momentCapture ? boardPlayers[momentCapture.boardId][oppositeColor(wizardOrientation)] : "Opponent";
  const studyUi = Boolean(store.game && studyUiEnabled);
  const exitStudyUi = () => {
    localStorage.removeItem("thejimmyapp.ui");
    const browserUrl = new URL(location.href);
    browserUrl.searchParams.delete("ui");
    history.replaceState(null, "", `${browserUrl.pathname}${browserUrl.search}${browserUrl.hash}`);
    setStudyUiEnabled(false);
  };
  const analysisContent = <FakeStockfishGate isGuest={Boolean(store.guestMatch)} analysisUnlocked={guestSession.analysis_unlocked} savedMomentCount={momentCount} guestNumber={guestSession.guest_number}>
    <LiveEvalCard gameLoaded={Boolean(store.game)} storedGameId={store.game && !store.guestMatch ? Number(store.game.game.id) : null} guestMatchId={store.guestMatch?.game_ids.A ?? null} globalPly={store.globalPly} board={stagedSourceBoard} boardName={stagedBoardName} position={stagedPosition} onSendLineToMoment={store.guestMatch ? openEngineLineMoment : undefined} />
  </FakeStockfishGate>;
  const dockActions = store.game ? <>
    ...
    {view === "review" && <button disabled={guestCoachLocked} className={`coach-button ${guestCoachLocked ? "capability-locked" : ""}`} title={guestCoachLocked ? "Team Coach unlocks in a future guest capability" : "Run the coupled Bughouse coaching pipeline"} onClick={() => setCoachOpen(true)}><Bot size={16} /> Team Coach{guestCoachLocked && <LockKeyhole className="capability-lock-badge" size={10} aria-hidden="true" />}</button>}
  </> : undefined;
  return (
    <>
    <AppShell
      ...
      rail={<>
        ...
        <nav className="rail-nav" aria-label="Main views">
          ...
          <button className={view === "notes" ? "active" : ""} aria-label="Notes board" title="Notes board" onClick={() => setView("notes")}><StickyNote size={17} /></button>
        </nav>
      ...
      stage={showOnboarding ? (onboardingPhase === "landing" ? <LandingPage completed={guestSession.completed} showAccountActions={!questTopBarVisible && !guestSession.completed} onStart={startFromLanding} /> : <GuestMatchupList onSelect={selectGuestMatch} />) : studyUi ? <StudyWorkspace onSaveMoment={store.guestMatch ? openMomentEditor : undefined} onOpenLibrary={() => setGuestLibraryOpen(true)} savedMomentCount={momentCount} saveMomentDisabled={!captureMomentContext(store.guestMatch, store.game, store.globalPly)} collaborate={dockActions} collaboratePanel={<CollaboratePanel active locked={capabilityLocked("dock_collaborate")} />} analysis={analysisContent} onClassicView={exitStudyUi} /> : view === "review" ? <section className="workspace">
      ...
      </section> : view === "stats" ? <StatsDashboard username={store.username} /> : <NotesBoard />}
      suppressDock={studyUi}
      dock={<SidePanel capabilities={guestProgress.capabilities} initialTab="review" activeBoard={activeReviewBoard} boardFocusEnabled={Boolean(store.guestMatch)} onActiveBoardChange={setActiveReviewBoard} dockBoardName={dockBoardName} onSwapBoards={store.guestMatch ? () => setBoardsSwapped((current) => !current) : undefined} onSelectGame={selectGame} loadingGame={gameMutation.isPending} savedLessons={guestProgress.savedLessons} savedMoments={guestProgress.savedMoments} savedMomentCount={momentCount} questCompleted={guestProgress.questCompleted} questProgress={questProgress} roomQuestRemainingSeconds={roomQuestRemaining} momentPlayers={momentPlayers} qualifyingGames={qualifyingGames} onOpenSavedLesson={openSavedLesson} onRemoveSavedLesson={removeSavedLesson} onOpenSavedMoment={openSavedMoment} onCopySavedMoment={copySavedMomentLink} onRemoveSavedMoment={removeSavedMoment} infoContent={reviewInfo} analysisContent={analysisContent} dockActions={dockActions} boardContent={store.game ? <BoardPanel boardId={dockSourceBoard} position={dockPosition} orientation={dockOrientation} pieceStyle={pieceStyle} layout="compact" beforeAnalyze={beforeAnalyze} analysisLocked={guestAnalysisLocked} keyboardFocused={Boolean(store.guestMatch) && activeReviewBoard === "B"} title={dockBoardName} playerTop={dockBoardAvailable ? dockPlayerTop : "Diagonal Opponent Unknown"} playerBottom={dockBoardAvailable ? dockPlayerBottom : "Partner Unknown"} unavailable={!dockBoardAvailable} onImportBothBoards={openImport} externalFallbackUrl={currentGameFallbackUrl} /> : undefined} />}
    />
      {guestLibraryOpen && document.getElementById("app-stage-panel") && createPortal(<GuestFlashcardPanel guestNumber={guestSession.guest_number} remainingSeconds={questRemaining} questCompleted={guestProgress.questCompleted} completionRecorded={guestSession.completed} account={accountQuery.data?.account ?? null} accountLoading={accountQuery.isPending} onClaimAccount={api.claimAccount} onAccountClaimed={(account) => queryClient.setQueryData(["account"], { account })} onClose={() => setGuestLibraryOpen(false)} />, document.getElementById("app-stage-panel")!)}
    {store.game && <TeamCoach open={coachOpen} onClose={() => setCoachOpen(false)} boardA={sourceBoardA} boardB={sourceBoardB} />}
      {momentCapture && wizardSourceFrame && wizardPosition && wizardPairedPosition && document.getElementById("app-stage-panel") && createPortal(
          ...
          <AnnotationWizardShell
            ...
            render_alternative_board={(onMovePlayed) => (
              <BoardPanel
                ...
                pairedPosition={wizardPairedPosition}
                orientation={wizardOrientation}
                pieceStyle={pieceStyle}
                title={`Board ${momentCapture.boardId} alternative`}
                playerTop={wizardPlayerTop}
                playerBottom={wizardPlayerBottom}
                layout="standard"
                onMoveIntent={async (intent) => {
                  const result = await api.explorationMove({
                    board_a_fen: wizardSourceFrame.board_a.variant_fen,
                    board_b_fen: wizardSourceFrame.board_b.variant_fen,
                    board: momentCapture.boardId,
                    from_square: intent.from,
                    to_square: intent.to,
                    drop_piece: intent.dropPiece,
                  });
                  const notation = result.notation?.trim();
                  if (result.legal && notation) onMovePlayed(notation);
                  if (result.legal && !notation) return { ...result, legal: false, reason: "The validated move did not include notation." };
                  return result;
                }}
              />
            )}
        ...
        document.getElementById("app-stage-panel")!,
      )}
      {!momentCapture && momentPersistenceError && <div className="moment-persistence-error" role="alert">{momentPersistenceError}</div>}
      {momentPermalinkError && <div className="moment-permalink-error" role="alert">{momentPermalinkError}</div>}
